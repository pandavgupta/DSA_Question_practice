...
Node* creat_BST(int pre[], int size, int start = 0){
    if(start >= size) return nullptr;
    Node* root = newNode(pre[start]);
    int index =start+1;
    while(index < size && pre[start] > pre[index]) index++;
    root->left = creat_BST(pre, index, start+1);
    root->right = creat_BST(pre, size, index );
    return root;
}
Node* post_order(int pre[], int size)
{
    return creat_BST(pre, size,0);
}
